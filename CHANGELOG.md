...
**Fixed**
- **The working copy was one commit behind `origin/nextjs`**, missing v2.378's polling denylist. Both scheduled jobs — `com.kabatone.seo-weekly` (Mon 08:07) and `com.kabatone.seo-geo` — run with `WorkingDirectory` set to that copy, so **this morning's Slack brief was generated by the superseded scorer** and put a rank-tracking bot back at the top of the opportunity table. The same bot drove three of the four P0 actions in the 2026-08-04 brief. `fc4db54`'s own commit body predicted this exact failure on 2026-09-08: *"will not take effect on Monday until that working copy is restored or the jobs are repointed."* The OneDrive outage that caused it resolved; the pull never happened.
- Rebased onto `origin/nextjs`. Conflicts were the three predicted append-at-top files (`CHANGELOG.md`, `SEO/carry-over.md`, `changelog.html`); the scorer files did not conflict. Verified by execution, not by reading: `best fire computer aided dispatch software` now returns `polling` / 0.0 and sits at **rank 56 of 56** with its label visible, rather than being silently dropped — the same treatment `c5` gets.
- **`scripts/gsc_pull_weekly.py` — two different implementations, both correct.** The tracked file (v2.378's reconstruction, which shells out to the `seo` skill helper) collided with an untracked local original (direct OAuth2) that had rehydrated from OneDrive after being presumed lost. The original was moved aside, not deleted. The tracked version was then tested against the same 28-day window and reproduced it exactly — **613 clicks / 121,698 impressions / 98 striking-distance**. They agree; the tracked one stands.

**Added**
- `SEO/weekly-report-2026-09-14.md` + `SEO/gsc-fresh-2026-09-14.json`. Organic **+19%** (1,008 sessions, 718 users), clicks +15.4%. GEO **68.0% → 84.0% on the identical 25-query set**, four flips N→Y and none regressing — including `best fire computer aided dispatch software`, which is the test v2.343 set for its fire-dispatch section. That query is simultaneously worthless for clicks and a valid GEO win; both are true.
- `AUTH-2` flipped to cited **with no authority spent**, which undercuts the premise that it was blocked on backlinks. Flagged for re-examination before budget is allocated.

**Changed**
- `SEO/carry-over.md` — `SYNC-1` logged and resolved same-day. `CAD-1` annotated: it was already **P0 #1 on 2026-09-08** and has not moved, making this its second consecutive week as the leading content action. It was demoted to P1 this week only because SYNC-1 corrects the table CAD-1's own priority is computed from.
...
- `best fire computer aided dispatch software` topped the 2026-09-08 opportunity table at 120
  "qualified" clicks. Its impressions are generated by a rank-tracking bot: **2,007 of 2,010
  desktop**, 3 mobile, **0 clicks at position 6.8**, and a fixed usa/gbr/nld/deu/ita/hkg country
  spread with **zero Mexico** — our actual market. Unfiltered, this same set drove three of the
  four P0 actions in the 2026-08-04 brief.
- Added `_SERP_POLLING` to the shared scorer with a new `polling` intent scoring **0.0**, so the
  Slack brief, the dashboard and the weekly report all correct at once.
- Deliberately a **denylist, not a threshold.** "Page-1 position, high impressions, zero clicks"
  equally describes genuine AI-Overview absorption — 16 real queries in this pull. A threshold
  would have suppressed one of the most important findings the programme has.
- Added `verify_serp_polling()`, which pulls the device and country split for a query so the
  list is extended with evidence rather than suspicion.

**Fixed — the GEO citation rate averaged two runs on one date**
- 2026-08-31 held 37 rows for 25 distinct queries and published **73.0%**, a number belonging to
  neither run. `geo_freshness()` now de-duplicates by `(date, query)`, last run wins, and reports
  `superseded_rows`. That date now reads **68.0%** (17/25), with 12 rows superseded.
- Like-for-like on the 12 queries common to both runs: **83.3% in each** — flat against 08-28,
  not the 10-point fall the raw row count implied. There was no GEO decline.
- **This was not the v2.340 double-run bug recurring**, as a draft of this week's report claimed.
  That fix worked: 24 doubled rows at 08:07 (`fa2a289`) were correctly reduced to 12 by 09:04
  (`4e3a3af`). The extra rows came from `geo-queries.txt` growing 14 → 42 lines at 13:31
  (`9fa95d9`) and the expanded set running on top. Two honest runs, one date, no wasted spend.
  The real defect is that the history carries no run identifier.

**Added**
- `scripts/gsc_pull_weekly.py` is now **tracked**. It existed only as an untracked file in the
  OneDrive working copy, which became unreadable on 2026-09-08 (every read `ETIMEDOUT`, 12,602 of
  13,304 files dehydrated), taking the weekly pull with it. Reconstructed and validated by
  replaying the 2026-09-01 window: striking-distance count reproduced exactly (101), pages 20/20,
  queries 29/30, every difference traced to Search Console backfill.
- Known inherited quirk, preserved deliberately: `top_queries_by_impressions` and
  `top_pages_by_impressions` are both sorted by **clicks**. The names are wrong; renaming them
  would break every stored pull.
...
**Added**
- **`scripts/scheduler-watchdog.mjs` (new, v1.0) + `~/Library/LaunchAgents/com.kabatone.seo-scheduler-watchdog.plist`.** Every coverage detector we had ran *inside* the scheduler: `seo-audit.mjs`'s `checkMissedRuns()` only speaks when an audit runs, and `routine-health.mjs`'s LOST DAYS scan joins run history against the runlog, so an outage — which produces no run rows to join — is invisible to it **by construction**. The watchdog is a `launchd` job with no dependency on Paperclip: it reads the newest dated line in `scripts/seo-daily-runlog.txt`, and if that is more than 36h stale it fires a macOS notification, appends to `~/.claude/tasks/scheduler-watchdog.log`, and exits 1. Exit 2 (runlog missing/unparseable) *also* alerts — a watchdog that goes quiet when it cannot see is worse than none.
- **A dead-man ping, the only leg that works while the Mac is off.** On each healthy check the script pings an external uptime URL read from `~/.config/claude-seo/healthcheck-url`; on a stale check it pings `/fail` instead, so the external service alerts immediately rather than waiting out its grace period. Pinging only when *coverage* is fresh is deliberate — pinging unconditionally would degrade this into "is the machine on", which is not the question. Currently reported as **INACTIVE** in every run, with the reason printed, until that URL file exists.

**Fixed**
- **Root cause of the outages: the machine was off, not the scheduler.** `pmset -g log` shows the MacBook draining on battery through 2026-08-22, reaching `Charge:1%` and hibernating at **2026-08-23 02:46 +0300**, with no further power event until `Wake from Hibernate ... Using AC` at **2026-08-27 11:27 +0300**. Paperclip's own hourly DB backups corroborate it to the hour: last pre-gap backup `20260822-191055`, first post-gap `20260827-042917`, nothing between. So `com.kabatone.paperclip`'s `KeepAlive` was already correct and irrelevant, and no in-instance retry could ever have helped. This closes the "needs separate diagnosis" note left open in v2.326.
- **`RunAtLoad` is the key that matters, not `StartInterval`.** Nothing on this Mac can alert while the Mac is dead, so the local leg's job is to alert the *instant* it comes back — at login/boot, before anyone thinks to open a runlog. `StartInterval 900` exists to keep the dead-man ping frequent enough for a 1d-period / 12h-grace check and to re-check through the day; re-alerting is rate-limited to 12h inside the script, and the cooldown is cleared on recovery so the next outage alerts at once.

**Notes**
- **Verified by replaying the real outage.** Run against the untouched runlog with `--now` set across the gap, the watchdog reports OK on 08-23 (21h), STALE on 08-24 (45h, ~1 day uncovered), STALE on 08-26 (93h, ~3 days) and OK again on 08-27. The notification path, the log path and the 12h cooldown suppression were each exercised for real, not asserted. The LaunchAgent is bootstrapped and its first run exited 0.
- **Future-dated runlog lines are excluded rather than trusted.** Taking the raw maximum would let a single clock-skewed or hand-backfilled line report the loop as covered indefinitely — a watchdog silenceable by a typo.
- **The 2026-08-05 → 08-11 window cannot be confirmed the same way**: `pmset` retains only back to 2026-08-20. The backup trail jumps `20260804` → `20260816`, which is consistent with the same machine-down cause but is not conclusive, because backup retention prunes. Stated as suggestive, not proven.
- **Catch-up on restart is deliberately *not* automated here.** Auto-creating a recovery audit needs a durable Paperclip API key on disk and duplicates the routine's own claim logic; the alert names the action instead. Raised as an explicit choice rather than silently deciding it.
- **This does not fix `adapter_failed` (KAB-2883)** — that is 3 of the 11 lost days and lives in platform code outside this repo. This covers the other 8.
