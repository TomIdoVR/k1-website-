...
# ── Automated SERP polling ────────────────────────────────────────────────────
# A third class, distinct from intent: queries whose impressions are generated by a
# rank-tracking bot rather than by people. They present as the most attractive rows on
# the board -- page-1 position, thousands of impressions, and a CTR gap of ~100% -- so an
# unfiltered scorer ranks them first. Unfiltered, this exact set drove three of the four
# P0 actions in the 2026-08-04 brief, and `best fire computer aided dispatch software`
# returned to #1 on 2026-09-08 at 120 "qualified" clicks.
#
# The fingerprint is mechanical and was verified per query against GSC:
#   ~100% desktop  ·  0 clicks at a page-1 position  ·  a fixed country spread
#   (usa/gbr/nld/deu/ita/hkg) with ZERO Mexico -- our actual market.
#   e.g. 2026-09-06: 2,007 of 2,010 impressions desktop, 3 mobile, 0 clicks at pos 6.8.
#
# This is a denylist, NOT a threshold, and that is deliberate. "Page-1 position with high
# impressions and zero clicks" also describes genuine AI-Overview absorption -- 16 real
# queries in the 2026-09-08 pull. A threshold would suppress that finding, which is one
# of the most important signals the programme has. Only add a query here with the
# device/country evidence in hand; verify_serp_polling() produces it.
_SERP_POLLING = {
    'best fire computer aided dispatch software',
    'computer automated dispatch software',
    'peregrine.ai analytics reporting dashboards',
}


def is_serp_polling(query):
    """True for queries whose impressions are bot-generated. Evidence required to add."""
    return query.lower().strip() in _SERP_POLLING


def verify_serp_polling(query, start_date, end_date, prop='https://kabatone.com/'):
    """Pull the device and country split for one query so a candidate can be added to
    _SERP_POLLING with evidence rather than suspicion. Returns a dict; does not decide."""
    import subprocess, json as _json
    helper = str(Path.home() / '.claude' / 'skills' / 'seo' / 'scripts' / 'gsc_query.py')
    out = {'query': query, 'device': {}, 'country': {}}
    for dim, key in (('device', ('desktop', 'mobile', 'tablet')),
        ...
        for v in key:
            cmd = ['python3.11', helper, '--property', prop, '--start-date', start_date,
                   ...
                   '--json', '--' + dim, v]
            try:
                rows = _json.loads(subprocess.run(cmd, capture_output=True, text=True,
                                                  check=True).stdout).get('rows', [])
            except Exception:
                continue
            m = {r['keys'][0]: r for r in rows}.get(query)
            if m:
                out[dim][v] = {'impressions': m['impressions'], 'clicks': m['clicks']}
    return out
...
def business_value(query):
    ...
    # Bot-generated impressions are worth nothing and cannot be won. Zero, not 0.2 --
    # a navigational query at least represents a person.
    if intent == 'polling':
        return 0.0
