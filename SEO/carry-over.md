...
| SHIP-1 | Get the SEO fixes onto production — **via PR from `main`, not a merge of `nextjs`** | 2026-08-21 | Omer | blocked | **Corrected 2026-08-31.** Earlier entries said "production is 13 commits behind". That was a one-directional count. `main` is simultaneously **163 commits ahead** on a separate line at a *higher* version (production v2.375 vs staging v2.337) — v2.338–v2.375 reached production as PRs #12–#16 and never went through `nextjs`. Merging `nextjs` into `main` would merge an older line into a newer one across **8 files modified on both sides** (`next.config.ts`, `metadata.ts`, `k-video`, `what-is-video-management-software`, …) — real regression risk. The underlying need is real: production's `next.config.ts` has zero references to the v2.329 redirects. The fix is a PR branched from `main` with the SEO changes re-applied and the 8 overlaps resolved against `main`'s newer versions. Not started — needs Omer, since that flow appears to be owned elsewhere. |
